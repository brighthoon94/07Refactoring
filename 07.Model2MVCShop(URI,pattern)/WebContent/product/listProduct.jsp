		...
		<form name="detailForm" action="/product/listProduct?menu=${param.menu }"
				...
				<tr>
					...
					<td background="/images/ct_ttl_img02.gif" width="100%"
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td width="93%" class="ct_ttl01">${param.menu == 'manage' ? '상품관리':'상품목록조회'}</td>
								</tr>	
						</table>
					</td>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				<tr>
					...
					</select> <input type="text" name="searchKeyword" value="${! empty search.searchKeyword ? search.searchKeyword : "" }"  class="ct_input_g" style="width: 200px; height: 19px" /></td>
						...
						<table border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td background="/images/ct_btnbg02.gif" class="ct_btn01"
									...
									<a href="javascript:fncGetPageList('1');">검색</a></td>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				<c:forEach var="product" items="${list}">
					...
					<td align="left">
						...
						<c:if test= "${param.menu == 'search' && !empty product.proTranCode && (user.role == 'admin') }">
						<a href="/product/getProduct?prodNo=${product.prodNo }&menu=${param.menu}">${product.prodName }</a>
						</c:if>
						<c:if test= "${param.menu == 'search' && product.proTranCode == '0' && (empty user.role || user.role == 'user')}">
						<a href="/product/getProduct?prodNo=${product.prodNo }&menu=${param.menu}">${product.prodName }</a>
						</c:if>
						<c:if test="${param.menu == 'search' && product.proTranCode != '0' && (empty user.role || user.role == 'user')}">
							${product.prodName}
